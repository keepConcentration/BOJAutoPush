...
<p>delete 키를 누를 때마다 $D$만큼의 에너지가 소비되고, 마우스로 드래그할 때마다 드래그한 문자열의 길이에 상관없이 $M$만큼의 에너지가 소비된다. 하지만, 김한양은 키보드를 내려치느라 힘이 다 빠져버렸기 때문에, 문자들을 지우는 데에 사용되는 에너지를 최소화하고자 한다.</p>
...
### 입력 
 ...
 <p>첫 번째 줄에 문자열 $S$의 길이를 나타내는 정수 $N\ (1 \leq N \leq 200\,000)$이 주어진다.</p>

<p>두 번째 줄에 메모장에 적힌 문자열 $S$가 주어진다. $S$는 알파벳 소문자와 대문자로만 이루어져 있다.</p>

<p>세 번째 줄에 $D\ (1 \leq D \leq 100)$와 $M\ (1 \leq M \leq 100)$이 공백으로 구분되어 주어진다.</p>

### 출력 

 <p>첫 번째 줄에 김한양이 소비한 에너지의 최솟값을 출력한다. 소비한 에너지가 $0$이라면 <span style="color:#e74c3c;"><code>Nalmeok</code></span>을 출력한다.</p>

<p>두 번째 줄에 김한양이 만들 수 있는 HYU의 최대 개수를 출력한다. 만들 수 있는 HYU의 개수가 $0$이라면 <span style="color:#e74c3c;"><code>I love HanYang University</code></span>를 출력한다.</p>
